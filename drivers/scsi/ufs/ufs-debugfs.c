...
static int ufsdbg_dump_configration_desc_show(struct seq_file *file, void *data)
{
	int err = 0;
	...
	struct desc_field_offset configration_desc_field_name[] = {
		{"bLength",		0x00, BYTE},
	...
	pm_runtime_get_sync(hba->dev);
	err = ufshcd_read_configration_desc(hba, desc_buf, buff_len);
	pm_runtime_put_sync(hba->dev);
	...
	if (!err) {
		...
		for (i = 0; i < ARRAY_SIZE(configration_desc_field_name); ++i) {
			tmp = &configration_desc_field_name[i];

			if (tmp->width_byte == BYTE) {
				seq_printf(file,
					   "Configration Descriptor[Byte offset 0x%x]: %s = 0x%x\n",
					   tmp->offset,
					   tmp->name,
				...
				seq_printf(file,
				"Configration Descriptor[offset 0x%x]: %s. Wrong Width = %d",
				tmp->offset, tmp->name, tmp->width_byte);
			}
		}
	} else {
		seq_printf(file, "Reading Configration Descriptor failed. err = %d\n",
			   err);
	}
...
static int ufsdbg_dump_configration_desc_open(struct inode *inode, struct file *file)
{

	return single_open(file,
			   ufsdbg_dump_configration_desc_show, inode->i_private);
}

static const struct file_operations ufsdbg_dump_configration_desc = {
	.open		= ufsdbg_dump_configration_desc_open,
	.read		= seq_read,
};
...
void ufsdbg_add_debugfs(struct ufs_hba *hba)
{
	...
	hba->debugfs_files.dump_configration_desc =
		debugfs_create_file("dump_configration_desc", S_IRUGO,
				    hba->debugfs_files.debugfs_root, hba,
				    &ufsdbg_dump_configration_desc);
	if (!hba->debugfs_files.dump_configration_desc) {
		dev_err(hba->dev,
			"%s:  NULL dump_configration_desc file, exiting", __func__);
		goto err;
	}
